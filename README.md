...
![Azure DevOps tests](https://img.shields.io/azure-devops/tests/eamonnerbonne/ValueUtils/1.svg) ![Azure DevOps coverage](https://img.shields.io/azure-devops/coverage/eamonnerbonne/ValueUtils/1.svg)

[ValueUtils](http://eamonnerbonne.github.io/ValueUtils/) implements `Equals` and `GetHashCode` for you.  By using runtime code-generation, the perf	SACCAscwq	csormance overhead is kept small; `ValueObject<>` generally outperforms alternatives such as `Tuple<>`, `struct` or anonymous types (see benchmarks below). 
